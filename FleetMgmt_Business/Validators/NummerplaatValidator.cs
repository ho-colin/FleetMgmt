...
namespace FleetManagement.Checkers {
    public static class NummerplaatValidator {
        ...
        public static bool isGeldig(string teValideren) {
            ...
            if (teValideren.Length != 9) throw new NummerplaatException("Nummerplaat moet 9 karakters lang zijn, Voorbeeld: 1-AAA-123");

            //Splitsen opgegeven string door koppelteken
            string[] gesplitstDoorKoppelteken = teValideren.Split('-');
            //array[0] is indexcijfer (1-9)
            if (gesplitstDoorKoppelteken[0].Length != 1 || !gesplitstDoorKoppelteken[0].All(char.IsDigit)) throw new NummerplaatException("Indexcijfer moet van 1 tot en met 9 zijn!");
